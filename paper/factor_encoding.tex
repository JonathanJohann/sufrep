...
In the  \emph{hermite} setup, outcome is linear in a sparse hermite polynomial basis $\phi(X_{i})$ where only $p$ randomly selected coefficients are active.
\begin{align}
  Y_{i} &= \phi(X_{i})^{T}\beta_{\ell} + \epsilon_{i} \label{eq:hermite_outcome}
\end{align}
