...
\begin{proof} \label{proof:lowrank}
  The proof is similar to the the previous one. However, this time note that we can decompose $\Omega = U D V^{T}$ using singular value decomposition, where the matrices have dimensions $|\mathcal{G}| \times |\mathcal{G}|$, $|G|\times p$, and $p \times p$ respectively. Letting $u(g) : g \mapsto (U)_{g, \cdot}^{T}$, we can write
  \begin{align}
    \psi(g) = A^\dagger u(g)^{T} D V^{T}
  \end{align}
  Once again, we separate out the part that depends on the category $g$. Letting $u(g): g \mapsto (U)_{g,1:k}$ row of matrix

  \begin{equation}
  \label{eq:a}
    \mu(x, \, g) &= \frac{\sum_{l = 1}^k  \EE{Y_i \cond X_i = x, \, L_i = l} \PP{X_i = x \cond L_i = l} (A^\dagger u(g)^{T} D_{1:k,1:k} V^{T})_l}{\sum_{l = 1}^k \PP{X_i = x \cond L_i = l} (A^\dagger \omega(g))_l}
  \end{equation}
