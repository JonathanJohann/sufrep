...
\begin{equation}
\label{eq:lowrank}
  \mu(x, \, g) = \frac{\sum_{l = 1}^k  \EE{Y_i \cond X_i = x, \, L_i = l} \PP{X_i = x \cond L_i = l} (A^\dagger u(g)^{T} D V^{T})_l}{\sum_{l = 1}^k \PP{X_i = x \cond L_i = l} (A^\dagger u(g)^{T} D V^{T})_l}
\end{equation}
\end{lemm}
...
\State $Z \gets \hOmega \cdot B_{\cdot,1:k}$
...
\begin{proof} \label{proof:lowrank}
  The proof is similar to the the previous one. However, this time note that we can decompose the $\Omega^T = U D V^{T}$ using singular value decomposition, where the matrices have dimensions $|\mathcal{G}| \times |\mathcal{G}|$, $|G|\times p$, and $p \times p$ respectively. Letting $u(g) : g \mapsto (U)_{g, \cdot}$, we can write
  \begin{align}
    \psi(g) = A^\dagger V D u(g)^T
  \end{align}
  such that $D$ and $V$ do not depend on the category $g$. We also note that the singular value decomposition of the transpose of a matrix A is simply $V D^T U^T$ and thus $V D u(g)^T$ in the way we have defined it is equivalent to $(\Omega)_{\cdot, g}$.
